...
<script lang="ts" setup>
  ...
  const getFileContents = () => {
    const client = getWebDAVClient();
    client
      .getFileContents(DB_PATH_CONFIG.WEBDAV.BOOKMARKS, { format: 'text' })
      .then((result: any) => {
  ...
  const putFileContents = () => {
    ...
    db.bookmarks.find({}, (err, data) => {
      client
        .putFileContents(DB_PATH_CONFIG.WEBDAV.BOOKMARKS, JSON.stringify(data), {
          overwrite: true,
        })
        .then((result: any) => {
  ...
  const getUploadUrl = () => {
    console.log('AAA');
    const client = createClient(import.meta.env.VITE_JIANGUOYUN_DAV_URL, {
  ...
  const test = async () => {
    ...
    let mainBaseDirItem = directoryList.find(
      (item: any) => item.basename === mainBaseDir && item.type === 'directory'
    );
    if (mainBaseDirItem) {
      console.log('exit ');
      const remote = client.createWriteStream(DB_PATH_CONFIG.WEBDAV.BOOKMARKS);

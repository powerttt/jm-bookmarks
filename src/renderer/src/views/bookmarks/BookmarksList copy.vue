<template>
  <div v-for="(itemTree, indexTree) in bookmarksTree" :key="indexTree">
    ...
    <div v-for="(item, index) in itemTree.children" :key="index" :id="item.uuid">
      ...
      <!-- ADD -->
      <n-grid :x-gap="12" :y-gap="6" cols="2 s:2 m:3 l:4 xl:6 2xl:7" responsive="screen">
        <n-grid-item>
          <n-card class="bookmarks-card bookmarks-add-card">
            <n-icon size="30">
              <Add28Regular />
            </n-icon>
          </n-card>
        ...
        <n-grid-item v-if="item.category == BookMarksItemCategory.BOOK_MARKS">
          ...
          >
            <div class="bookmarks-desc text-break">{{ item.desc }}</div>
            <template #header>
              <n-ellipsis line-clamp="2" :tooltip="false">
                <div :style="cardContentStyle">{{ item.name }}</div>
              </n-ellipsis>
            </template>
            <template #footer>
              <BookmarksListCardFooter :bookmarks="item" />
            </template>
          </n-card>
        </n-grid-item>
          ...
          >
            <template #header>
              <n-ellipsis line-clamp="3" :tooltip="false">
                <div :style="cardContentStyle">{{ cItem.name }}</div>
              </n-ellipsis>
            </template>
            <template #footer>
              <BookmarksListCardFooter :bookmarks="cItem" />
            </template>
            <template v-if="cItem.category === BookMarksItemCategory.DIR">
              <n-dropdown
                @select="handleSelect"
                trigger="hover"
                key-field="uuid"
              ...
              >
                <div>
                  <n-ellipsis line-clamp="2" :tooltip="false">
                    <div :style="cardContentStyle">{{ cItem.desc }}</div>
                  </n-ellipsis>
                </div>
              </n-dropdown>
            </template>

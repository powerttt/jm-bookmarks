...
<script lang="ts" setup>
  import {
    BookMarksItem,
    BookMarksItemCategory,
    getBookMarksItemDefaultValue,
    JmDatastore,
  } from '../../types';
  ...
  const renderLabel = ({ option }) => {
    return h('div', { onClick: () => clickLabel(option) }, { default: () => option.name });
  };
  ...
  const handleDrop = ({ node, dragNode, dropPosition }) => {
    ...
    if (dropPosition === 'inside') {
      ...
      const [nodeSiblings, nodeIndex] = bookmarksFindSiblingsAndIndex(node, props.bookmarksTree);
      // 拖动到某个标签后面
